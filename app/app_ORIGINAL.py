...
def upload_image():
    ...
    if ai_model is not None:
        try:
            # أ. تحميل الصورة وتغيير حجمها
            img = Image.open(filepath)
            img = img.resize((180, 180))  # نفس أبعاد التدريب
            
            # ب. تحويل الصورة إلى مصفوفة numpy وتسويتها
            img_array = np.array(img) / 255.0
            img_array = np.expand_dims(img_array, axis=0)  # شكل: (1, 180, 180, 3)
